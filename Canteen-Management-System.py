...
if 'users' not in st.session_state:
    st.session_state.users = {}  # Store users with hashed passwords
...
def login(username, password):
    if username in st.session_state.users and verify_password(st.session_state.users[username], password):
...
if st.session_state.current_user is None:
    ...
    auth_option = st.sidebar.selectbox("Select an option", ["Login", "Register"])
    ...
    if auth_option == "Register":
        ...
        if st.button("Register"):
            result = register(reg_username, reg_password)
            st.write(result)
        ...
        if st.button("Login"):
            result = login(log_username, log_password)
            st.write(result)
else:
    # Main app UI
    st.sidebar.title("Navigation")
    option = st.sidebar.selectbox("Select an option", [
        "Add Snack", "Remove Snack", "Update Availability", "Record Sale", "Display Inventory", "Display Sales Records", "Logout"])
